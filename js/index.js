require([
  ...
  const issExternalRenderer = {
    ...
    setup: function(context) {
      ...
      // load ISS mesh
      const issMeshUrl = "assets/Porsche_911_GT2.obj";
      ...
      esriRequest("//open-notify-api.herokuapp.com/iss-now.json", {
        ...
        .then(
          function(response) {
            ...
            let vel = [0, 0];
            if (this.positionHistory.length > 0) {
              const last = this.positionHistory[
                this.positionHistory.length - 1
              ];
              const deltaT = result.timestamp - last.time;
              const vLon =
                (result.iss_position.longitude - last.pos[0]) / deltaT;
              const vLat =
                (result.iss_position.latitude - last.pos[1]) / deltaT;
              vel = [vLon, vLat];
            }
            ...
            if (this.positionHistory.length >= 2) {
              const entry = this.positionHistory[
